var box1 = new ReconnectingWebSocket(location.protocol.replace("http","ws") + "//" + location.host + "/ws/chat1");

box1.onmessage = function(message) {
...
box1.onclose = function(){
    console.log('box1 closed');
    this.box1 = new WebSocket(box1.url);
};
...
$("#input-form").on("submit", function(event) {
  ...
  box1.send(JSON.stringify({ handle: handle, text: text }));
  $("#input-text")[0].value = "";
});
